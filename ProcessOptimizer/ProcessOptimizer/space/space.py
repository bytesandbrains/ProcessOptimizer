...
class Space(object):
    ...
    def lhssample(self, n_init):
        '''
        Compute n_init random samples made by homebrew'y latin_hyper_sampling
        returns as list of random points spread in an structured randomly manner
        '''
        p = len(self.dimensions)
        x = np.random.uniform(size=[n_init,p])
        for i in range(0,p):
            x[:,i] = (np.argsort(x[:,i])+0.5)/n_init
        samples_in_space = x * np.abs(np.diff(self.bounds).T) + np.asarray(self.bounds).min(axis=1)
